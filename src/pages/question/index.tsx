...
const Question: React.FC = () => {
  ...
  const language = i18n.language === 'ru' ? 'russian' : 'romanian';

  const filterQuestion =
    videoQuestions?.length > 0
      ? videoQuestions
          ?.filter(
            (item: { type: string[]; lan: string }) =>
              item?.type?.includes(state?.selectionId) && item?.lan === language
          )
  ...
  return (
    <>
      <Layout>
        <>
          <h1 className="my-5 text-[22px] font-bold text-primary">
            {`${t('question.header')}  ${currentIndex + 1}`}
          </h1>
      ...
      {blocker.state === 'blocked' ? (
        <div
          className="relative z-[9999999999999999999999]"
