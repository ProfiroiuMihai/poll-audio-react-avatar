    ...
    <div className="flex h-[90%] flex-col items-center justify-center">
      ...
      {i18n.language === 'ru' ? (
        <div className="mt-5 flex items-center gap-2 text-xs">
          ...
          <CiPlay1 />
          <p> чтобы прослушать ответ</p>
        </div>
      ) : (
        <div className="mt-5 flex items-center gap-2 text-xs">
          <p>Apasa pe butonul</p>
          <CiPlay1 />
          <p>de mai sus, pentru a asculta răspunsul</p>
        </div>
      )}
    </div>
  ) : (
    <div className="mt-[45%] flex h-[90%] flex-col items-center justify-center">
      {capturing ? (
        <button
          ...
          aria-label="stop"
          className="relative flex h-20 w-20 items-center justify-center rounded-full bg-warning"
        >
      ...
      {capturing ? (
        <p className="mt-10 text-center text-secondary">
          {t('record.recordStopBtnText')}
        </p>
      ) : (
        <p className="mt-10 text-center text-secondary">
          {t('record.recordBtnText')}
        </p>
      )}
    </div>
  );
